...
def socket_send(ip,javadir,agent,zone):
    ...
    return a
def socket_sendgamedata(ip,gamedbdata,gamedbport,agent,zone):
    md5command = "md5sum /var/ftp/gamedata/game_data.sql"
    status,output = commands.getstatusoutput(md5command)
    filesmd5 = re.split(" ",output)[0]
    sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    sock.connect(ip,1002)
    remote_command = "update_gamedata,dfjedfei15ADFEpoi9340dAEgee0e,%s,%s,%s" %(gamedbdata,gamedbport,filesmd5)
    sock.send(remote_command)
    result = sock.recv(2048)
    sock.close()
    if int(result) == 0:
...
def list(request):
    ...
    if request.method == 'POST':
        ...
        servers = ServerManger.objects.values('ip','gamedbdata','gamedbport','agent','zone','javadir','wwwdir').filter(zone__in=server_list)
        for item in servers:
            if action == u'更新版本':
    ...
    #return render_to_response('list.html',{'list': server_mangers},context_instance=RequestContext(request))
                ...
                print 'gamedbdata is %s' % item['gamedbdata']
                res = pool.apply_async(socket_sendgamedata,(item['ip'],item['gamedbdata'],item['gamedbport'],agent,item['zone']))
                result.append(res.get(timeout=20))
            return render_to_response('list.html',{'result':result},context_instance=RequestContext(request))
        return render_to_response('list.html',{'list': server_mangers},context_instance=RequestContext(request))
def view(request):
...
def view3(request):
    ...
    return render_to_response('list1.html',RequestContext(request,{'list':list}))
def view4(request):
    list = ServerManger.objects.all().order_by('zone')
